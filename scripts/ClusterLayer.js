define([
  ...
  return declare([GraphicsLayer], {
    constructor: function(options) {
      ...
      this._fullExtent = [99999999, 99999999, -99999999, -99999999];
    ...
    _clusterAddPoint: function(p, cluster) {
      ...
      var oldMatchType = cluster.attributes.matchType;
      var newMatchType = p.attributes.MatchType;
      
      if (oldMatchType == newMatchType) {
          if (newMatchType == "Related") {
              cluster.attributes.matchType = "MULTI_RELATED";
          } else if (newMatchType == "Identical") {
              cluster.attributes.matchType = "MULTI_IDENTICAL";
          }
      } else if (oldMatchType.indexOf("MULTI") > -1) {
          if (oldMatchType == "MULTI_RELATED" && newMatchType == "Identical") {
              cluster.attributes.matchType = "MULTI_IDENTICAL_RELATED";
          } else if (oldMatchType == "MULTI_IDENTICAL" && newMatchType == "Related") {
              cluster.attributes.matchType = "MULTI_IDENTICAL_RELATED";
          }
      } else if (oldMatchType != newMatchType) {
          if (oldMatchType == "Related" && newMatchType == "Identical") {
              cluster.attributes.matchType = "MULTI_IDENTICAL_RELATED";
          } else if (oldMatchType == "Identical" && newMatchType == "Related") {
              cluster.attributes.matchType = "MULTI_IDENTICAL_RELATED";
          }
      }
    ...
    _clusterCreate: function(p) {
      ...
      var cluster = { 
        "x": p.x,
        "y": p.y,
        "type": p.attributes.MatchType,
        "attributes": {
          "matchType": p.attributes.MatchType,
          "clusterCount": 1,
          "clusterId": clusterId,
    ...
    _showAllClusters: function() {
      for ( var i = 0, il = this._clusters.length; i < il; i++ ) {
        ...
        if (c.attributes.extent[0] < this._fullExtent[0]) {
            this._fullExtent[0] = c.attributes.extent[0];
        }
        if (c.attributes.extent[2] > this._fullExtent[2]) {
            this._fullExtent[2] = c.attributes.extent[2];
        }
        if (c.attributes.extent[1] < this._fullExtent[1]) {
            this._fullExtent[1] = c.attributes.extent[1];
        }
        if (c.attributes.extent[3] > this._fullExtent[3]) {
            this._fullExtent[3] = c.attributes.extent[3];
        }
